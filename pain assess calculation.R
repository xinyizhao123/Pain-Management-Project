...
## EXCEPTION 1: The only pre-intervention assess is for the previous intervention
...
test2.1$diff.pre1[test2.1$diff.pre1.i == 1] <- NA

## EXCEPTION 2: The only pre-intervention assess is for the previous intervention

## Example as below

# Pain Score Time    | Pain Intervention Time
# ------------------ | ----------------------
# Null               | 2016/01/01 8:00 AM
# 2016/01/01 8:00 AM | Null
# Null               | 2016/01/01 8:30 AM (case)
# 2016/01/01 9:00 AM | Null
...
test2.1 <- test2.1[,diff.pre1.ii:=as.numeric(shift(Pain.Intervention.Time,2,type="lag")==shift(Pain.Score.Time,1,type="lag")),by=CSN...Shift]
test2.1$diff.pre1.ii[is.na(test2.1$Pain.Intervention.Time)] <- NA
test2.1$diff.pre1[test2.1$diff.pre1.ii == 1] <- NA
...
# ------------------ | ----------------------
# 2016/01/01 8:00 AM | 2016/01/01 8:00 AM
# 2016/01/01 8:30 AM | 2016/01/01 8:30 AM (case)
# 2016/01/01 9:00 AM | Null
...
#### Possible case 3 -- Eligible assess time is in the 3rd next row (could be due to entry error)

## Example as below

# Pain Score Time    | Pain Intervention Time
# ------------------ | ----------------------
# Null               | 2016/01/01 8:00 AM (case)
# 2016/01/01 8:00 AM | Null
# Null               | 2016/01/01 8:30 AM
# 2016/01/01 8:30 AM | Null

# diff time from the 3rd next row

test2.1 <- test2.1[,diff.post3:=round((shift(Pain.Score.Time,3,type="lead")-Pain.Intervention.Time)*24*60,digits=0),by=Patient.CSN.ID]
test2.1$diff.post3[test2.1$diff.post3 <= 0] <- NA


#### Possible case 4 -- Eligible assess time is in the 4th or more next row

# diff time from the 4th next row

test2.1 <- test2.1[,diff.post4:=round((shift(Pain.Score.Time,4,type="lead")-Pain.Intervention.Time)*24*60,digits=0),by=Patient.CSN.ID]
test2.1$diff.post4[test2.1$diff.post4 <= 0] <- NA

# diff time from the 5th next row

test2.1 <- test2.1[,diff.post5:=round((shift(Pain.Score.Time,5,type="lead")-Pain.Intervention.Time)*24*60,digits=0),by=Patient.CSN.ID]
test2.1$diff.post5[test2.1$diff.post5 <= 0] <- NA

# diff time from the 6th next row

test2.1 <- test2.1[,diff.post6:=round((shift(Pain.Score.Time,6,type="lead")-Pain.Intervention.Time)*24*60,digits=0),by=Patient.CSN.ID]
test2.1$diff.post6[test2.1$diff.post6 <= 0] <- NA


#### compliance -- If any of the eligible diff time <= 60 then yes

test2.1$post.compliance <- as.numeric(test2.1$diff.post1 <= 60 | test2.1$diff.post2 <= 60 | test2.1$diff.post3 <= 60 | test2.1$diff.post4 <= 60 | test2.1$diff.post5 <= 60 | test2.1$diff.post6 <= 60)
